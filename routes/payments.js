...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!invoice_id || !amount || !payment_date) {
      await conn.rollback();
      return res.status(400).json({ success: false, message: 'Invoice, amount and date required.' });
    }
    ...
    const [result] = await conn.query(
      `INSERT INTO payments (company_id, invoice_id, party_id, amount, payment_date, payment_mode, reference_no, notes)
       VALUES (?,?,?,?,?,?,?,?)`,
      [req.companyId, invoice_id, invoice.party_id, amount, payment_date,
    ...
    const [updated] = await query('SELECT * FROM invoices WHERE id = ?', [invoice_id]);
    res.status(201).json({
      success: true,
